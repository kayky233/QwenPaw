...
def test_parse_remote_head_symbolic_ref_accepts_short_and_full_forms() -> None:
    assert parse_remote_head_symbolic_ref("origin/develop\n") == "develop"
    assert (
        parse_remote_head_symbolic_ref("refs/remotes/origin/release/v2")
        == "release/v2"
    )
...
def test_validate_git_branch_name_rejects_unsafe_values(branch: str) -> None:
...
@pytest.mark.asyncio
async def test_resolve_default_branch_falls_back_to_remote_advertisement(
    tmp_path: Path,
) -> None:
    ...
    branch = await resolve_remote_default_branch(run_process, tmp_path)
...
async def test_resolve_default_branch_falls_back_to_remote_refs(
    ...
    assert await resolve_remote_default_branch(run_process, tmp_path) == "master"


def test_build_research_branch_uses_issue_and_bounded_plan_suffix() -> None:
    dialog = _dialog()

    assert build_research_branch(dialog) == "autoresearch/issue-42-planabc1"


def test_build_research_branch_uses_task_when_issue_is_absent() -> None:
    dialog = _dialog(goal="Develop a memory TTL feature", plan_markdown="# Plan")

    assert build_research_branch(dialog) == "autoresearch/issue-task-planabc1"


def test_reusable_worktree_requires_all_resume_metadata(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_prepare_reuses_preserved_worktree_without_git_commands(
    tmp_path: Path,
) -> None:
    worktree = tmp_path / "worktree"
    worktree.mkdir()
...
async def test_prepare_local_repository_uses_remote_default_branch(
    ...
    assert worktree == (
        package_root / ".qwenpaw" / "worktrees" / "research-planabc1"
    )
...
def test_real_router_uses_extracted_worktree_service() -> None:
    ...
    assert research_module._build_research_branch is build_research_branch
...
def test_installer_exposes_worktree_helpers() -> None:
    ...
    assert module._validate_git_branch_name is validate_git_branch_name
