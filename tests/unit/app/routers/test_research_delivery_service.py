...
def test_validate_github_repository_name_rejects_unsafe_values(
    ...
    with pytest.raises(RuntimeError, match="Invalid GitHub repository"):
...
def test_dialog_pr_head_uses_plain_branch_in_same_owner_repository() -> None:
    dialog = _dialog(push_repository="agentscope-ai/QwenPaw")

    assert dialog_pr_head(dialog) == "autoresearch/issue-42-plandel1"


def test_dialog_pr_title_prefers_explicit_issue_number() -> None:
    assert dialog_pr_title(_dialog()) == "fix: resolve issue #42"
...
def test_dialog_pr_title_normalizes_non_issue_goal() -> None:
    ...
    assert dialog_pr_title(dialog) == "fix: Add memory TTL support"
...
def test_build_pull_request_payload_binds_dynamic_base_branch() -> None:
    ...
    assert payload == {
        ...
        "head": "kayky233:autoresearch/issue-42-plandel1",
        "base": "develop",
        "body": "# Evidence",
    }
...
@pytest.mark.asyncio
async def test_resolve_delivery_base_branch_uses_cached_worktree_branch(
    tmp_path: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_resolve_delivery_base_branch_recovers_from_git_remote_head(
    tmp_path: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_push_research_branch_validates_and_pushes(tmp_path: Path) -> None:
...
async def test_create_dialog_pr_cli_uses_dynamic_base_branch(
    ...
    dialog = _dialog()
    run_process = AsyncMock(return_value="https://github.com/agentscope-ai/QwenPaw/pull/9\n")
    ...
    pr_url = await create_dialog_pr(
        ...
        executable_lookup=lambda name: "/usr/bin/gh" if name == "gh" else None,
        environment={},
    )

    assert pr_url == "https://github.com/agentscope-ai/QwenPaw/pull/9"
    ...
    assert args[args.index("--title") + 1] == "fix: resolve issue #42"
    run_process.assert_awaited_once()


@pytest.mark.asyncio
async def test_create_dialog_pr_rest_uses_same_payload_and_token(
    tmp_path: Path,
) -> None:
    captured: dict = {}

    def rest_creator(repository: str, payload: dict[str, str], token: str) -> str:
        captured.update(
            repository=repository,
            payload=payload,
            token=token,
        )
        return "https://github.com/agentscope-ai/QwenPaw/pull/10"
    ...
    assert captured["repository"] == "agentscope-ai/QwenPaw"
...
async def test_create_dialog_pr_requires_delivery_credentials(
    ...
    with pytest.raises(RuntimeError, match="install and authenticate GitHub CLI"):
...
def test_installer_exposes_delivery_helpers() -> None:
    ...
    assert module._build_pull_request_payload is build_pull_request_payload
...
def test_real_router_uses_extracted_delivery_service() -> None:
    ...
    assert research_module._build_pull_request_payload is build_pull_request_payload
